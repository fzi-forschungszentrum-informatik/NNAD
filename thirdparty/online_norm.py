...
class OnlineNorm(keras_layers.Layer):
    """
    ...
    def control_normalization(self, inputs):
        ...
        def forward(inputs):
            ...
            mu_b, var_b = tf.nn.moments(inputs, self.norm_ax)

            scale = tf.sqrt(self.var + self.epsilon)

            # get the new mean and variances
            new_mu = self.mu + (1 - self.alpha_fwd) * (mu_b - self.mu)
            new_var = (self.alpha_fwd * self.var + (1 - self.alpha_fwd) * var_b +
                        (self.alpha_fwd * (1 - self.alpha_fwd) * tf.square(mu_b - self.mu)))
            ...
            def grad(deltas):
                ...
                # control with u
                grad_delta = delta_temp / tf.reshape(scale, self.broadcast_shape) - (1 - self.alpha_bkw) * tf.reshape(self.u_ctrl, self.broadcast_shape)

                # update u control variables
                # update the estimate u controller which controls
                # orthogonality to the 1 vector
                self.u_ctrl.assign_add(
                    tf.reduce_mean(
                        grad_delta,
                        axis=self.norm_ax,
                        keepdims=False
    ...
    def layer_scaling(self, inputs):
        ...
        scale = tf.reduce_mean(inputs * inputs,
                               ...
                               keepdims=True)
        return inputs * tf.math.rsqrt(scale + self.epsilon)
