...
class ResnetModule(tf.keras.Model):
    ...
    def build(self, input_shapes):
        ...
        self.conv2 = tf.keras.layers.SeparableConv2D(self.num_filters[1],
            (3, 3),
            use_bias=False,
            padding='same',
        ...
        self.conv3 = tf.keras.layers.Conv2D(self.num_filters[2],
            (1, 1),
            kernel_initializer=tf.keras.initializers.he_normal(),
            kernel_regularizer=tf.keras.regularizers.l2(L2_REGULARIZER_WEIGHT),
            name='conv3')
    ...
    def call(self, x, train_batch_norm=False):
        ...
        x = tf.keras.activations.relu(x)
        x = self.conv2(x)
        x = self.bn3(x, training=train_batch_norm)
        x = tf.keras.activations.relu(x)
        x = self.conv3(x)
        x = x + skip
        return x
...
class ResnetBackbone(tf.keras.Model):
    def __init__(self, name, pretrain=False):
        ...
        self.padding1 = tf.keras.layers.ZeroPadding2D((1, 1))
        self.maxpool1 = tf.keras.layers.MaxPooling2D(3, 2)

        self.module_1a = ResnetModule('resnet_module_1a', [64, 64, 128])
        self.module_1b = ResnetModule('resnet_module_1b', [64, 64, 128])
        self.module_1c = ResnetModule('resnet_module_1c', [64, 64, 128])
        ...
        if pretrain:
            ...
            dilation_rate = 2
        self.module_3a = ResnetModule('resnet_module_3a', [256, 256, 512], stride=stride,
...
class ResnetBranch(tf.keras.Model):
    def __init__(self, name):
        ...
        self.module_4a = ResnetModule('resnet_module_4a', [512, 512, 1024], dilation_rate=2)
        self.module_4b = ResnetModule('resnet_module_4b', [512, 512, 1024], dilation_rate=2)
        self.module_4c = ResnetModule('resnet_module_4c', [512, 512, 1024])
